...
pub fn crackers(engine: &mut Engine, vertices: &Vec<Vertex>, shaders: ShaderModuleDescriptor) -> IndexedShape {
...
pub fn generate_rectangle_vertices(vertices: &Vec<Vertex>, index: usize) -> Vec<Vertex> {
    ...
    let b = Vertex { position: [ c.position[0],  a.position[1], 0.0], color: c.color };
    let d = Vertex { position: [ a.position[0],  c.position[1], 0.0], color: a.color };

    vec!(a, c, b, d)
}

//                                                                                (0, 1, *2*, *3*)
pub fn generate_rectangle_indices(index: usize) -> Vec<u16> {  // Awaited format: (A, C, *B*, *D*)
    let mut rectangle_layout = vec!(
        0, 1, 2,
        2, 3, 0,
    );
