...
use ignition::{manifestation::GPU, prelude::*};
...
pub fn record_command_buffer(c: &mut Criterion) {
    ...
    c.bench_function("Record command buffer", |b| {
        b.iter(|| {
            ...
            pollster::block_on(compare(&mut engine, &output_buffer, &image));
...
async fn compare(engine: &mut Engine<GPU>, output_buffer: &Buffer, image: &Vec<u8>) {
